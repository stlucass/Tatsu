  ...
  var ProductManager = (function(){
    ...
    */
    ...
    var updatePoduct = function(id, quantity) {
      ...
      var products = getAllProducts();
      products[productIndex].quantity = typeof quantity === "indefinido" ? products[productIndex].quantity * 1 + 1 : quantity;
      setAllProducts(products);
      return true;
    }
    var setProduct = function(id, name, summary, price, quantity, image) {
      if(typeof id === "undefined"){
        console.error("id necessário")
        return false;
      }
      if(typeof name === "undefined"){
        console.error("nome necessário")
        return false;
      }
      if(typeof image === "undefined"){
        console.error("imagem necessária")
        return false;
      }
      if(!$.isNumeric(price)){
        console.error("preço não é um número")
        return false;
      }
      if(!$.isNumeric(quantity)) {
        console.error("quantidade não é um número");
        return false;
      }
  ...
  var loadMyCartEvent = function(userOptions){
    ...
    if(!$("#" + idCartModal).length) {
      $('body').append(
        ...
        '<a type="button" href="pagamento.html" class="btn btn-primary" id="' + idCheckoutCart + '">Pagar</a>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>'
    ...
    var drawTable = function(){
      ...
      if(discountPrice !== null) {
        $cartTable.append(
          ...
          '<td></td>' +
          '<td><strong>Total (desconto de primeira compra)</strong></td>' +
          '<td></td>' +
          '<td></td>' +
